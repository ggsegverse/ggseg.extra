...
- Pipeline intermediates are stamped with `cache_format_version()`
  ...
  stamped.
- When a change makes existing intermediates **wrong** rather than
  merely old — a fix to surface projection, label classification,
  contour extraction or anything else whose cached output would
  otherwise be reused — bump `cache_format_version()` in the same PR.
  Nothing detects a forgotten bump: the symptom is a maintainer
  rebuilding an atlas and silently getting the old one.
