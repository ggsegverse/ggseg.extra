...
#'
#' The grey outline drawn behind an atlas's structures is labelled `cortex`,
#' and almost every polish step treats it differently from the structures --
#' a silhouette wants rounding that keeps its sulci, the structures want
...
#' `"^cortex"`, so a change of convention is one release rather than one edit
#' per atlas repository.
#'
#' @details
#' Anchored and case-sensitive on purpose. A loose `"cortex"` also matches
#' `Cerebellar_Cortex_*` and `Left-Cerebral-Cortex`, which are structures, not
#' the silhouette.
#'
