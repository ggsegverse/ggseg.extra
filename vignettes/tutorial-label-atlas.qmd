...
format: html
vignette: >
  %\VignetteIndexEntry{Tutorial: Creating an atlas from label files}
...
This runs the same pipeline as `create_cortical_from_annotation()` --- it projects the inflated mesh triangles straight to 2D and converts them to sf polygons.
