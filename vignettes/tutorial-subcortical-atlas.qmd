...
`subcortical_slabs()` derives the projection slabs from the bounding box of a
...
to `slabs`, and run `aseg_context()` in the same call via `context`:
...
`create_wholebrain_from_volume()` forwards both `slabs` and `context` through
