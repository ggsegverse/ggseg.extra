...
`focus` is matched case-sensitively against the leftover labels, so a region is
never swallowed by a context entry that is a substring of its name --- the
classic `Thalamus` ⊃ `hypothalamus` trap that silently produced 0-region
atlases before.
