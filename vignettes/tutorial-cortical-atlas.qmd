...
The pipeline returns raw, unsmoothed polygons — smoothing is now a separate post-processing step via `atlas_smooth()`. Apply it once and tune `keep` freely; you no longer have to re-run the full pipeline to try a different simplification level:
