...
format: html
vignette: >
  %\VignetteIndexEntry{Tutorial: Creating a cortical atlas}
...
The pipeline returns raw, unsmoothed polygons. Tidying them is a separate post-processing step in two parts: `atlas_simplify()` drops vertices, `atlas_smooth()` rounds off what is left. Tune either freely; you no longer have to re-run the full pipeline to try a different level:
