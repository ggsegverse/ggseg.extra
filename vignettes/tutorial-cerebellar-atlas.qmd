...
Smoothing is a separate post-processing step via `atlas_smooth()` — apply it once and tune `keep` freely without re-running the pipeline:
...
The `tolerance`, `smoothness` and `smooth_refinements` arguments on the `create_*()` functions are deprecated.
Supplying them emits a lifecycle warning and they are otherwise ignored.
