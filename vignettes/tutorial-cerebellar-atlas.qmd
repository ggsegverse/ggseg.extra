...
The pipeline reads the GIFTI label table for region names and colours, maps labels onto the SUIT flatmap vertices, builds per-region sf polygons from the mesh triangles, and applies Chaikin smoothing to clean up jagged triangle boundaries.
...
The cerebellar pipeline has two parameters that control the 2D polygon geometry:

- `tolerance` — Douglas-Peucker simplification.
  Default is 1.
  Higher values produce simpler polygons with fewer vertices.
  Set to 0 for maximum fidelity.
- `smooth_refinements` — Chaikin corner-cutting iterations.
  Default is 2.
  More iterations produce rounder region boundaries.
